...
```
birthday: LocalDate?           // 연도를 모르면 대체 연도(2000, 윤년)로 저장
birthdayHasYear: Boolean       // 연도를 아는지
birthdayIsLunar: Boolean       // 음력인지
```

- 연도를 모르는 경우가 흔하다(3월 15일은 아는데 몇 년생인지는 모름). SQLite에 월-일 타입이
  없어서 대체 연도를 넣고 플래그로 구분한다. 2월 29일이 살아남도록 대체 연도는 윤년이다.
...
표시 위치: S2 헤더(수정 가능), S3 브리핑 최상단(7개 섹션 **위**, 읽기 전용).
브리핑에서 고정 정보는 "소식"이 아니라 "신원"이라 섹션이 아니라 머리말로 둔다.
...
| `생일:` | 고정 정보. `1990-03-15` / `3-15`(연도 모름) / `음력 3-15`. `.`과 `/`도 구분자로 받는다 |
