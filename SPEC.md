...
- **이름을 영역 안에 직접 쓴다.** 각 영역의 라벨 위치는 그 영역 안에서 경계로부터 가장 먼 점
  (최대 내접원의 중심)으로 계산한다. 영역은 원과 원 바깥의 교집합이므로 경계까지의 거리는
  각 원의 테두리까지 거리 중 최솟값과 정확히 같다 — 그래서 손으로 좌표를 맞출 필요가 없고
  배치를 바꿔도 라벨이 따라간다. 내접원 안에 들어가는 정사각형만 글상자로 쓰므로
  모서리까지 영역 안에 있다. 다 들어가지 않으면 "외 N명"으로 줄이고, 전체 명단은 그림 아래에 남긴다.
- 그림은 정사각형 영역에 그린다. x를 너비로, 반지름을 min(너비,높이)로 각각 환산하면
